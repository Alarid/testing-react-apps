...
Inline snapshots are particulary useful when it comes to making assertions on
error messages, for example. Instead of pasting the error message in the test,
we can use `toMachInlineSnapshot()` and update the snapshots when they fail
because the error message changed.

`@jackfranklin/test-data-bot` is a lib that makes it easier to generate fixtures
for tests

Instead of modifying our server handlers to simulate errors in specific case for
our test, it's better to use MSW `server.use` and create a one-off server
handler just for the test. Don't forget to clean after with
...
https://ws.kcd.im/?ws=Testing%20React%20Applications%20%F0%9F%A7%90&e=05%3A%20mocking%20HTTP%20requests&em=yoh.legrand%40gmail.com
